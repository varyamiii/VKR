...
## Обновление: авторизация учеников и преподавателей

В этой версии добавлены роли пользователей: ученик и преподаватель. Ученик может проходить учебный режим и тестирование, а преподаватель может просматривать агрегированные результаты и формировать сводный PDF-отчёт.

### Миграция существующей базы данных

Если база данных уже была создана старой версией проекта, выполните миграцию:

```powershell
psql -h localhost -p 5432 -U postgres -d vkr_atc_training -f .\db\upgrade_auth_teacher_reports.sql
```

Миграция создаёт таблицы `student_groups`, `students`, `teachers`, переименовывает runtime-таблицы тестирования с `test_session_*` на `test_attempt_*` и добавляет связь попытки тестирования с учеником.

### Демонстрационные пользователи

После выполнения миграции можно войти под тестовыми пользователями:
...
Пароли в БД хранятся не открытым текстом, а в виде PBKDF2-SHA256-хэша.

### Страницы

```text
/                  вводная страница выбора роли
/login/student     вход ученика
/login/teacher     вход преподавателя
/training          учебный режим ученика
/test/setup        настройка тестирования ученика
/teacher/dashboard отчёты преподавателя
```
